...
{"up":-0.40073136119999997,"down":0.5356747272,"left":0.9974491569999999,"right":-0.066575312}
		...
		*/
		...
		thresholdProportion:0.35, //determines the thresholds from limits
		...
		setup : function() {
			...
			this.$status = $(".status");
			var statusEls = []
			for( var p in this.statusProperties){
				statusEls.push('<li class="'+p+'" ><span class="property">'+p+'</span><span class="value"></span></li>');
			}
			this.$status.append(statusEls.join(''));
		...
		updateCalibPoint:function(t1,t2){
			...
			this.calibPoint = {
				xs: this.documentOffsetX + offset.left +  headOffset.left + (w/2), 
				ys: this.documentOffsetY + offset.top + headOffset.top + (h/2), 
				t1: t1,
				t2: t2
			};
		},
		...
		updateStatus:function(state){
			var me = this;
			this.$status.children().each(function(){
				var prop = $(this).find('.property').html();
				
				$(this).find('.value').html(state[prop]);
				var isValid = false;
				switch(me.statusProperties[prop]){
					case 1:
					case 0:
						isValid = state[prop] === me.statusProperties[prop];
						break;
					case '<=5':
						isValid = state[prop] <= 5;
					break;
					case '[]':
						isValid = state[prop].length === 0;
					break;
				}
				if(isValid){
