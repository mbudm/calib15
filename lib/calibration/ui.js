...
{"up":-0.40073136119999997,"down":0.5356747272,"left":0.9974491569999999,"right":-0.066575312}
		...
		*/
		...
		updateThresholds:function(){
			...
			//limit pitch to the same scale as yaw - because we have the excessYaw error to capture edge of movement
			// we also capture pitch center as a guage of comfortable cnter pitch
			this.limits[this.pitchMinProp] = this.currentCenter.pitch - (yawRange / 3);
			this.limits[this.pitchMaxProp] = this.currentCenter.pitch + (yawRange / 3);
		...
		normaliseAxis:function(axis,val){
			...
			//if the value is in the middle or error range then leave it as is.
			if(val > this.overExtend[maxProp] || val < this.overExtend[minProp] || ( val < this.snap[maxProp] && val > this.snap[minProp] )  ){
				adjVal = val;
			}else{
			...
			return ((adjVal - this.snap[minProp]) / this.getAxisRange(axis, 'overExtend') * 2) - 1; 
		},
