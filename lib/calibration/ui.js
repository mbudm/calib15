...
{"up":-0.40073136119999997,"down":0.5356747272,"left":0.9974491569999999,"right":-0.066575312}
		...
		*/
		...
		updateThresholds:function(){
				...
				this.snap[this.pitchMaxProp] = this.limits[this.pitchMaxProp] - (this.snapProportion * this.getPitchRange() );
				this.snap[this.pitchMinProp] = this.limits[this.pitchMinProp] + (this.snapProportion * this.getPitchRange() );
				this.snap[this.yawMaxProp] = this.limits[this.yawMaxProp] - (this.snapProportion * this.getYawRange() );
				this.snap[this.yawMinProp] = this.limits[this.yawMinProp] + (this.snapProportion * this.getYawRange() );
		...
		normalisePitch:function(p){
			var adjp = p > this.snap[this.pitchMaxProp] ? this.snap[this.pitchMaxProp] : (p < this.snap[this.pitchMinProp]? this.snap[this.pitchMinProp] : p ); 
			//console.log('adjp:'+adjp,this.limits);
			return ((adjp - this.snap[this.pitchMinProp]) / this.getPitchRange(true) * 2) - 1; 
		},
		normaliseYaw:function(y){
			var adjy = y > this.snap[this.yawMaxProp] ? this.snap[this.yawMaxProp] : (y < this.snap[this.yawMinProp] ? this.snap[this.yawMinProp]: y ); 
			return ((adjy - this.snap[this.yawMinProp]) / this.getYawRange(true) * 2) - 1; 
		},

		getPitchRange:function(snap){
			var max = snap ? this.snap[this.pitchMaxProp] : this.limits[this.pitchMaxProp] ;
			var min = snap ? this.snap[this.pitchMinProp] : this.limits[this.pitchMinProp] ;
			return max - min; 
		},

		getYawRange:function(snap){
			var max = snap ? this.snap[this.yawMaxProp] : this.limits[this.yawMaxProp] ;
			var min = snap ? this.snap[this.yawMinProp] : this.limits[this.yawMinProp] ;
			return max - min; 
		},
