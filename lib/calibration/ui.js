	...
	var calibUi = {
		...
		*/
		...
		updateHeadRange:function(){
			if(this.userUp === null){
				this.userUp = this.lastState.kvHeadPitch;
				this.userDown = this.lastState.kvHeadPitch;
				this.userRight = this.lastState.kvHeadYaw;
				this.userLeft = this.lastState.kvHeadYaw;
			}else{
				this.userUp = Math.max(this.lastState.kvHeadPitch, this.userUp);
				this.userDown = Math.min(this.lastState.kvHeadPitch, this.userDown);
				this.userLeft= Math.max(this.lastState.kvHeadYaw, this.userLeft);
				this.userRight = Math.min(this.lastState.kvHeadYaw, this.userRight);
			}
		},
		...
		checkInstructions:function(){
			...
			if(timeElapsed > this.instructions[this.instructionsIdx].delay){
				...
				if(this.instructions[this.instructionsIdx].task){

					this[this.instructionsIdx].task.method](this.instructionsIdx].task.param);
		...
		setCalibPosition:function(param){
			this.$calibDot.addClass('calibDot-'+param);
		}
