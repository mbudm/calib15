...
xLabs.calib.ui = {
	...
	*/
		...
		validationCheckErrors:['f','r'],//,'r'], // the only one we are worrying about for now (plus position)
		...
		updateCalibPoint:function(t1,t2){
			...
			this.calibPoint = {
				xs: xLabs.api.documentOffsetX + headOffset.left + (w/2), 
				ys: xLabs.api.documentOffsetY + headOffset.top + (h/2), 
				t1: t1,
				t2: t2
			};
		},
		...
		positionUserHead:function(pitch,yaw){
			...
			var removes = [],
				...
				yaw = parseFloat(yaw),
				p = this.isNumeric(pitch) ? pitch : this.lastData[this.pitchProp], //(this.movingAverages[this.calibDotMovingAverage].pitch  || this.lastData[this.pitchProp]),
				yw = this.isNumeric(yaw) ? yaw : this.lastData[this.yawProp]; //(this.movingAverages[this.calibDotMovingAverage].yaw  || this.lastData[this.yawProp]);
		...
		logData:function(data){
		    ...
		    var slimData = {
				state:{
					...
					gaze:{
						estimate:data.state.gaze.estimate
					},
		...
		updateGazeDotPosition:function(){
			var x = this.lastData.state.gaze.estimate.y - xLabs.api.documentOffsetX;
			var y = this.lastData.state.gaze.estimate.x - xLabs.api.documentOffsetY;
		...
		checkEnvironment:function(ma){
			...
			if(this.categoriseAxis('yaw',this.movingAverages[ma].yaw,(this.yawRange * 2) ) !== "center" && this.categoriseAxis('pitch',this.movingAverages[ma].pitch,(this.pitchRange * 2) ) ){
