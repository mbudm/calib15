	...
	var calibUi = {
		...
		setUpInstructions:function(){
			this.getCurrentInstruction().start = Date.now();
			
			console.log("set up instructions", this.seqIndices, this.getCurrentInstruction());
			
			//default - calib message - is no to have msgTarget property
			if(!this.getCurrentInstruction().msgTarget){
				var adds = this.getMessageClasses(this.getInstructions()[this.getInstructionIdx()]);
		...
		checkEnvironment:function(ma){
			...
			if(Math.abs(this.movingAverages[ma].x) > this.positionCheckThreshold && Math.abs(this.movingAverages[ma].y) > this.positionCheckThreshold){
			...
			console.log(this.movingAverages[ma].val, this.movingAverages[ma].x, this.movingAverages[ma].y, this.movingAverages[ma], msgList, overallStatus);
		...
		updateFeedbackStatus:function(msgList){
			...
			for(var i = 0; i < msgList.length; i++){
				if($children.length === 0){
					//add status items
					this.$feedbackMessages.append('<li>'+this.getFeedbackStatusMessage(msgList[i])+'</li>')
				}else{
					//update status items html
					$children.eq(i).html(this.getFeedbackStatusMessage(msgList[i]));
				}
			}
		},
