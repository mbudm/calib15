	...
	var calibUi = {
		...
		scaleErrorLevel:0.1,//when the user head dot turns red, also used to expand the radius
		thresholdProportion:0.25, //determines the thresholds from limits ( excluding error level )
		...
		validationCheckErrors:['f','u'], // the only one we are worrying about for now (plus position)
		...
		updateThresholds:function(){
			...
			*/
			...
			this.currentCenter.pitch = this.getArrayAverage([	this.limits[this.pitchMinProp] , this.limits[this.pitchMaxProp]]);
		...
		positionUserHead:function(pitch,yaw){
			...
			var removes = [],
				...
				yaw = parseFloat(yaw),
				p = this.isNumeric(pitch) ? pitch : (this.movingAverages[this.calibDotMovingAverage].pitch  || this.lastState[this.pitchProp]),
				yw = this.isNumeric(yaw) ? yaw : (this.movingAverages[this.calibDotMovingAverage].yaw  || this.lastState[this.yawProp]);
		...
		normaliseAxis:function(axis,val){
			...
			if(val > this.overExtend[maxProp]){
				...
				adjVal = val > this.currentCenter[axis] ? ( Math.min(val,this.snap[maxProp]) / this.snap[maxProp] * this.overExtend[maxProp] ) : ( Math.max(val,this.snap[minProp]) / this.snap[minProp] * this.overExtend[minProp] ) ;
			}
			*/
			/*if( val < this.snap[maxProp] && val > this.snap[minProp] ){
				//if the value is in the middle then scale it to the edge of the circle.
				adjVal = val > this.currentCenter[axis] ? (val / this.snap[maxProp] * this.snap[maxProp] ) : (val / this.snap[minProp] * this.snap[minProp] ) ;
	
			}else{
				//otherwise snap it the edge of the circle
				adjVal = val >= this.snap[maxProp] ? this.overExtend[maxProp] : this.snap[minProp] ;
				
			}*/

			var r = (  ( (val - this.limits[minProp]) / this.getAxisRange(axis, 'limits') * 2 ) - 1  ); // -1 to 1 ;
			if(r < -1 || r > 1){
				console.log(axis,'v:'+val,'result:'+r, 'min:'+this.limits[minProp] , 'range:'+this.getAxisRange(axis, 'limits'));
			}
			return  r;
		},
		...
		update:function(state){
			...
			if((this.simMode || state.kvRealtimeEnabled === 1 ) && state.kvTrackingEnabled === 1 ){
				...
				if(this.debuggersVisible){
					...
					this.$debug.val([JSON.stringify(this.limits), 
						JSON.stringify(this.overExtend), 
						JSON.stringify(this.snap),
						JSON.stringify(this.thresholds),
						JSON.stringify(this.currentCenter),
					JSON.stringify(this.movingAverages)].join('  \n\n  '));
