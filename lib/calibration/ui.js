	...
	var calibUi = {
		...
		*/
		...
		thresholdProportion:0.2, //determines the thresholds from limits
		...
		updateThresholds:function(){
				...
				this.limits.up = Math.max(this.lastState.kvHeadPitch, this.limits.up );
				this.limits.down = Math.min(this.lastState.kvHeadPitch, this.limits.down);
		...
		normalisePitch:function(p){
			return (p - this.limits.up) / this.getPitchRange() ; 
		},
	
		normaliseYaw:function(y){
			return (y - this.limits.right) / this.getYawRange(); 
		},

		getPitchRange:function(){
			return (this.limits.down - this.limits.up); // down is higher value
		},

		getYawRange:function(){
			return (this.limits.left - this.limits.right); // left is higher value
		},

		positionUserHead:function(){
			// categorise pitch and yaw
			var removes = [],
				adds = [],
				state = this.lastState,
				pitch = this.movingAverages[this.calibDotMovingAverage].pitch,
				yaw = this.movingAverages[this.calibDotMovingAverage].yaw;
			
			var radius = this.$calibDot.width();
			var pitchPx = Math.round(radius * this.normalisePitch(pitch)) + 'px';
			var yawPx = Math.round(radius * this.normaliseYaw(yaw) )+'px';

			this.$userHead.css({bottom:pitchPx, right:yawPx });
		},
