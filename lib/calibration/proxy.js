...
xLabs.calib.proxy = {
		...
		setCalibrationCapture:function(bool){
			if(!this.simulate){
				console.log('setCalibrationCapture',bool, Date.now())
				xLabs.api.setTruthEnabled( bool );
			}
		},
		
		calibrate:function(bool){
			if(!this.simulate){
				console.log('doCalibration', Date.now())
				xLabs.api.calibrate( );
			}
		},
		...
		*/
		sendCalibrationData:function(calibPoint){
			if(!this.simulate){
				console.log('calib point sent',calibPoint, Date.now());
				xLabs.api.setTruthScreen( calibPoint.x, calibPoint.y );
			}
		},
